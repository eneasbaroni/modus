const clientes = [
  {
    id: '2025ED36',
    username: 'marcos',
    pass: '1234',
    nombre: 'Marcos Ahumada',
    empresa: 'Quo',
    email: 'marcosahumada@gmail.com',
    telefono: '+34654321234',
    informes : [
      {
        fecha: '13-09-2023',        
        informe: 'https://drive.google.com/file/d/1MX43I-eo0bUfM82fraEQwAxv5qIXmSGt/view?usp=sharing',
        multimedia: 'https://drive.google.com/file/d/1omIWLax2m-le3Md_ecyxj1EgwF5bp-tU/view?usp=sharing',
        estadistica: 'https://drive.google.com/file/d/1X8Xo8V5ZkxbhNz-fKgRZGq7O3Y4cW9Ny/view?usp=sharing',
        grafico: 'https://www.solverglobal.com/wp-content/uploads/2021/12/PowerBI_GL_Quickstart_24-scaled-1.jpg'
      },
      {
        fecha: '13-08-2023',        
        informe: 'https://drive.google.com/file/d/18iehj1TXRdApQS8GS4m9gLMA-tRtAaxg/view?usp=sharing',
        multimedia: 'https://drive.google.com/file/d/132cgRhELwpD85vhDFO958DzGepfd0KCG/view?usp=sharing',
        estadistica: 'https://drive.google.com/file/d/1X8Xo8V5ZkxbhNz-fKgRZGq7O3Y4cW9Ny/view?usp=sharing',
        grafico: 'https://www.solverglobal.com/wp-content/uploads/2021/12/PowerBI_GL_Quickstart_24-scaled-1.jpg'
      },
      {
        fecha: '13-07-2023',        
        informe: 'https://drive.google.com/file/d/1ZwOEkoElNQDpoEciaLWDs88ubJCTETHR/view?usp=sharing',
        multimedia: 'https://drive.google.com/file/d/1GwyfKGo_Y15RlTN-IGQnLpe86JB_aYxh/view?usp=sharing',
        estadistica: 'https://drive.google.com/file/d/1X8Xo8V5ZkxbhNz-fKgRZGq7O3Y4cW9Ny/view?usp=sharing',
        grafico: 'https://www.solverglobal.com/wp-content/uploads/2021/12/PowerBI_GL_Quickstart_24-scaled-1.jpg'
      }
    ],
    mensajes : [
      {
        fecha : '2023-09-14',
        mensaje: 'Ya está disponible el informe de Agosto',
        leido: false
      },
      {
        fecha : '2023-08-14',
        mensaje: 'Ya está disponible el informe de Julio',
        leido: true
      },
      {
        fecha : '2023-07-14',
        mensaje: 'Ya está disponible el informe de Junio',
        leido: true
      },
    ]
  },

  {
    id: '2025ED37',
    username: 'juan',
    pass: '1234',
    nombre: 'Juan Perez',
    empresa: 'Empresa de prueba',
    email: 'juan@gmail.com',
    telefono: '+34654321234',
    informes : [
      {
        fecha: '13-09-2023',        
        informe: 'https://drive.google.com/file/d/1X8Xo8V5ZkxbhNz-fKgRZGq7O3Y4cW9Ny/view?usp=sharing',
        multimedia: 'https://drive.google.com/file/d/1X8Xo8V5ZkxbhNz-fKgRZGq7O3Y4cW9Ny/view?usp=sharing',
        estadistica: 'https://drive.google.com/file/d/1X8Xo8V5ZkxbhNz-fKgRZGq7O3Y4cW9Ny/view?usp=sharing',
        grafico: 'https://www.solverglobal.com/wp-content/uploads/2021/12/PowerBI_GL_Quickstart_24-scaled-1.jpg'
      }
    ],
    mensajes : [
      {
        fecha : '2023-09-14',
        mensaje: 'Ya está disponible el informe de Agosto',
        leido: false
      }
    ]
  },

  {
    id: '2025ED36',
    username: 'lorena',
    pass: '1234',
    nombre: 'Lorena Garcia',
    empresa: 'Empresa de prueba',
    email: 'loregarcia@gmail.com',
    telefono: '+34654321234',
    informes : [
      {
        fecha: '13-09-2023',        
        informe: 'https://drive.google.com/file/d/1X8Xo8V5ZkxbhNz-fKgRZGq7O3Y4cW9Ny/view?usp=sharing',
        multimedia: 'https://drive.google.com/file/d/1X8Xo8V5ZkxbhNz-fKgRZGq7O3Y4cW9Ny/view?usp=sharing',
        estadistica: 'https://drive.google.com/file/d/1X8Xo8V5ZkxbhNz-fKgRZGq7O3Y4cW9Ny/view?usp=sharing',
        grafico: 'https://www.solverglobal.com/wp-content/uploads/2021/12/PowerBI_GL_Quickstart_24-scaled-1.jpg'
      }
    ],
    mensajes : [
      {
        fecha : '2023-09-14',
        mensaje: 'Ya está disponible el informe de Agosto',
        leido: false
      }
    ]
  },
]

export default clientes